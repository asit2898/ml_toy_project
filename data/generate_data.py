...
def create_eval_datasets(data_save_dir: str, n_valid:int = 20000, n_test:int = 10000) -> None:
	'''
	...
	print(f'Generating validation and test data with {n_valid} and {n_test} samples respectively')
	...
	np.save(os.path.join(data_save_dir, 'validation_data_images.npy'), validation_data_images)
	np.save(os.path.join(data_save_dir, 'validation_data_circles.npy'), validation_data_circles)
...
def generate_dataset(
	...
	for i in range(length):
		image, circle = next(generate_examples(noise_level, img_size, min_radius, max_radius))
		images.append(image)
...
if __name__ == '__main__':
	...
	parser.add_argument('--data_save_dir', type=str, default='datasets', help='directory to save the datasets to')
	parser.add_argument('--n_valid', type=int, default=20000, help='size of the validation dataset')
	parser.add_argument('--n_test', type=int, default=10000, help='size of the test dataset')

	args = parser.parse_args()

	create_eval_datasets(data_save_dir=args.data_save_dir, n_valid= args.n_valid, n_test=args.n_test)
